...
class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        for i1 in range(board_size): # row
            freqMap1 = defaultdict(int)
            for k1 in range(board_size): # column
                current = board[i1][k1]
        ...
        # ----------------- Check 2 -----------------
        for i2 in range(board_size): # column
            freqMap2 = defaultdict(int)
            for k2 in range(board_size): # row
                current = board[k2][i2]
